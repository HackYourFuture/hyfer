  ...
  render() {
    ...
    if (searchMember.length) {
      filterMembers = filterMembers
        .filter((el) => el.full_name.toLowerCase().match(searchMember))
    } else if (filterByGroupName) {
      filterMembers = filterMembers.filter((el) => el.group_name === filterByGroupName)
              ...
              <ul>
                ...
                <li className={Styles.headerList}>
                  <div className={Styles.groupsSelected}>
                    <select title='Filter By Class name'
                      onChange={(e) => handleFieldChange(e, 'filterByGroupName')}
                      name="studentList" className={Styles.selectMenu}>
                      <option value='All'>Select Class</option>
                      {
                        members.map(member => member.group_name)
                          .filter((elem, pos, arr) => {
                            return arr.indexOf(elem) === pos;
                          }).filter(elem => elem !== null)
                          .map(groupName => {
                            return (
                              <option key={groupName}
                                value={groupName}
                              >
                                {groupName}
                              </option>
                ...
                <li>
                  <div>
                    <button title='Click to Add Student'
                      className={noAvailableTickets? Styles.disabledBtn : Styles.selectedBtn}

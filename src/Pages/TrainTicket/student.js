...
class Student extends Component {
  render() {
    const { member,
      handelSelected } = this.props;
    return (
      <div>
        <ul className={Styles.studentContent} key={member.id}>
          <li><span className={Styles.sName}>{member.full_name}</span></li>
          <li className={Styles.imgContainer}>
            <img className={Styles.img}
              src={`https://avatars.githubusercontent.com/${member.username}`} alt={member.full_name} />
          </li>
          <li>
            <input type="checkbox"

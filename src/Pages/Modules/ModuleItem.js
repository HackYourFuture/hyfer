...
export default class ModuleItem extends Component {
    ...
    EditModule = () => {
        this.setState({ isMenuShow : false , isEditing:true })
    }
        ...
        return (
            ...
            >
                <div className={style.moduleItemContent}>
                    <md-icon class="material-icons" onClick={this.showMenu}>more_vert</md-icon>
                    {module.module_name}{module.optional===1?"*":""}
                </div>
                <ul onMouseDown={(e)=>{e.stopPropagation()}} className={style.moduleMenu} 
                ...
                opacity:this.state.isMenuShow?"1":"0"}}>
                    ...
                    <li>
                        <button style={{color:module.ref_count > 0? "": "#FF5722"}} disabled={module.ref_count > 0?true:false} onClick={this.DeleteModule}>
                            <md-icon class="material-icons">delete</md-icon>
                            <span>Delete</span>
                        </button>
                    </li>
                </ul>
                <div onMouseDown={this.hideMenu} className={style.menuOverlay} style={{display:this.state.isMenuShow?"block":"none"}}></div>
            </Resizable>

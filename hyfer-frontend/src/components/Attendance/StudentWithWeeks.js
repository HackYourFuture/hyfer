...
export default class StudentWithWeeks extends React.Component {
  ...
  renderWeeks() {
    const { history, students } = this.props.modulesInfoStore;
    let studentHistory;
  ...
  getIsHomeworkChecked = (id, student) => {
    return this.props.modulesInfoStore.history[student][id].homework === 1;
  };

  getIsAttendanceChecked = (id, student) => {
    return this.props.modulesInfoStore.history[student][id].attendance === 1;
  };
}

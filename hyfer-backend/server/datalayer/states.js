...
const GET_CURRENT_STATE = `
  SELECT group_id, user_id, u.full_name, u.username, DATE_FORMAT(g.starting_date, "%Y-%m-%d") as date, g.group_name 
    FROM group_students 
...
const ASSIGN_USER_TO_CLASS = 'REPLACE INTO group_students VALUES ?';

function getStudentsState(con, groupId) {
  return execQuery(con, GET_CURRENT_STATE, groupId);
}

function updateUser(con, user) {
  return execQuery(con, UPDATE_USER, [user, user.id]);
}
